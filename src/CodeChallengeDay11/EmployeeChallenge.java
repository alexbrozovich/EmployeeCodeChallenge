	...
	public static ArrayList<Employee> createEmployeeList() {
		...
		try {
			...
			while(line != null) {
				...
				employeeArrayList.add(new Employee(singleEmployeeArray[0], Integer.parseInt(singleEmployeeArray[1].trim()), singleEmployeeArray[2]));
	...
	private static void getMoreInfo(Scanner scan, ArrayList<Employee> employeeList) {
		...
		for (Employee e: employeeList) {
				if( e.getName().equalsIgnoreCase(userIn)) {
					System.out.printf("Name: %s\nAge: %s\nFavorite Food:%s\n", e.getName(), e.getAge(), e.getFavoriteFood());;
					nameExists = true;
				}
